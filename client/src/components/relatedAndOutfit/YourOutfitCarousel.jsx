...
var YourOutfitCarousel = (props) => {
  var localOutfits;
  if (localStorage.getItem('outFits') !== null) {
    localOutfits = localStorage.getItem('outFits');
    localOutfits = localOutfits.split(',');
    for (var x = 0; x < localOutfits.length; x++) {
      localOutfits[x] = parseInt(localOutfits[x]);
    }
  ...
  function addOutfit() {
    console.log('LOCAL OUTFITS BEFORE ADDING IS:    ', localOutfits);
    localOutfits.push(parseInt(props.productId));
    localStorage.setItem('outFits', localOutfits);
    console.log('LOCAL OUTFITS IS NOW:   ', localOutfits);
  }
  function removingOutfit(e) {
    var newOutfit = [];
    for (var x = 0; x < localOutfits.length; x++) {
      if (e !== localOutfits[x]) {
        newOutfit.push(localOutfits[x]);
      }
    }
    if (newOutfit.length === 0) {
      localStorage.removeItem('outFits');
  ...
  var breakPoints = [
    ...
    <div>
      <h5 className="section-head">Your Outfit</h5>
      <Carousel breakPoints={breakPoints}>
        <YourOutfitCard handleClick={(e) => {addOutfit()}} />
        {localOutfits.map((product) => {
          return <YourOutfitCard key={product} productID={product} removingOutfit={(e) => {removingOutfit()}} />
        })}
      </Carousel>
    </div>
  );
